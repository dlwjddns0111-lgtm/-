                ...
                <div className="fixed inset-0 z-[100] bg-[#F9FAFB] dark:bg-gray-950 flex flex-col animate-in slide-in-from-right-full duration-300 max-w-lg mx-auto left-0 right-0 border-x border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 text-[13px] text-gray-600 dark:text-gray-400 leading-relaxed space-y-8 pb-24">
                        ...
                        <div className="space-y-8 px-1">
                            <PrivacySection title="제1조 (개인정보의 처리 목적)" content="사장님 인건비 계산기는 사용자의 효율적인 노무 관리를 돕기 위해 개인정보를 처리합니다. 수집된 정보는 앱 내 기능 제공 및 서비스 품질 개선 목적으로만 한정하여 사용됩니다." />
                            <PrivacySection title="제2조 (개인정보의 수집 항목)" content="서비스 제공을 위해 카카오로부터 고유 식별값, 닉네임, 프로필 사진을 제공받으며, 사용자가 직접 입력하는 직원 이름, 근무 일시, 급여 설정액 등을 수집합니다." />
                            <PrivacySection title="제3조 (개인정보의 보유 및 이용기간)" content="사용자가 데이터 삭제를 요청할 때까지 원칙적으로 보관됩니다. 다만, 관련 법령에 의해 보관이 필요한 경우 해당 기간 동안 별도 관리됩니다." />
                            <PrivacySection title="제4조 (정보주체의 권리 및 의무)" content="사용자는 언제든지 자신의 개인정보를 조회하거나 수정할 수 있으며, 앱 내 데이터 삭제를 통해 이용 동의를 철회할 수 있습니다. 수집된 데이터는 사용자의 명시적 요청 없이 제3자에게 제공되지 않습니다." />
                            <PrivacySection title="제5조 (개인정보 보호를 위한 기술적 대책)" content="사용자의 데이터를 보호하기 위해 모든 통신은 최신 암호화 기술(HTTPS/SSL)을 사용하며, Firebase 보안 규칙을 통해 본인의 데이터에만 접근 가능하도록 관리됩니다." />
                        </div>

                        <div className="pt-10 border-t border-gray-200 dark:border-gray-800 text-[11px] text-gray-400 font-black">
                            최종 업데이트: 2026년 5월 6일<br />
                            운영: 사장님 인건비 계산기 (support@payroll.app)
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeModal === 'help' && (
                <div className="fixed inset-0 z-[100] bg-[#F9FAFB] dark:bg-gray-950 flex flex-col animate-in slide-in-from-right-full duration-300 max-w-lg mx-auto left-0 right-0 border-x border-gray-100 dark:border-gray-800">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 pb-24 space-y-5">
                        <FaqItem question="주휴수당은 어떻게 계산되나요?" answer="일주일 소정근로시간이 15시간 이상이고, 모든 근무일에 개근한 경우 1일분의 임금을 추가로 지급하는 수당입니다. 이 앱은 해당 주의 총 근무시간을 기반으로 주휴수당을 자동 계산합니다." />
                        <FaqItem question="4대보험 공제 기준이 궁금해요." answer="2026년 기준 국민연금(4.75%), 건강보험(3.595%), 고용보험(0.9%)을 적용합니다. 장기요양보험료는 건강보험료의 13.134%입니다. 이는 근로자 부담분 기준이며 사장님 부담분은 별도입니다." />
                        <FaqItem question="밤 늦게 일하면 시급이 더 비싼가요?" answer="네, 밤 10시부터 다음 날 아침 6시 사이의 근로는 야간근로에 해당하여 통상임금의 50%를 가산해야 합니다. 직원 설정에서 야간수당을 켜면 자동으로 계산됩니다." />
                        <FaqItem question="휴게시간(점심시간)은 어떻게 빼나요?" answer="근무 기록 입력 시 '휴게시간' 항목에 분 단위로 입력하세요. 예를 들어 60분 입력 시 총 근무시간에서 1시간이 자동 차감됩니다." />
                        <FaqItem question="직원에게 급여 명세서를 보내고 싶어요." answer="정산 탭에서 직원 카드의 '상세 명세' 버튼을 누른 후, 노란색 [카톡 전송] 버튼을 누르면 급여 명세서가 카카오톡으로 바로 전송됩니다." />
                        <FaqItem question="로그인을 꼭 해야 하나요?" answer="로그인 없이도 사용 가능하지만, 데이터가 이 기기에만 저장됩니다. 다른 기기에서도 사용하거나 데이터를 안전하게 보관하려면 카카오 계정으로 로그인하세요." />
                        <FaqItem question="퇴직금 계산도 지원하나요?" answer="네, 지원합니다. 정산 탭의 '퇴직금' 탭에서 전 직원의 퇴직금 현황을 확인할 수 있습니다. 직원 입사일을 입력하면 근로자퇴직급여보장법 기준(1년 이상 근무 시 발생)으로 자동 계산됩니다." />
                        <FaqItem question="세금 3.3% 공제는 어떻게 하나요?" answer="직원 설정에서 4대보험을 끄면 프리랜서 기준 3.3%(소득세 3% + 지방소득세 0.3%)가 자동 적용됩니다." />
                    </div>
                </div>
            )}
        </div>
